...
public class Baekjoon_16935_w3 {
	...
	public static int[][] oper_4(int[][] arr) {
		int[][] new_arr = new int[m][n];
		for(int i=0;i<m;i++) {
			int[] tarr = new int[n];
			for(int j=0;j<n;j++) {
	...
	public static int[][] oper_5(int[][] arr) {
		int[][] new_arr = new int[n][m];
		int ch_n=n/2,ch_m=m/2;
		
		for(int i=0;i<ch_n;i++) {
			for(int j=0;j<ch_m;j++) {
				new_arr[i][j]=arr[i+ch_n][j];
				new_arr[i+ch_n][j+ch_m]=arr[i][j+ch_m];
			}
			for(int j=ch_m;j<m;j++) {
				new_arr[i][j]=arr[i][j-ch_m];
				new_arr[i+ch_n][j-ch_m]=arr[i+ch_n][j];
			}
		}
		
		return new_arr;
	}
	public static int[][] oper_6(int[][] arr) {
		int[][] new_arr = new int[n][m];
		int ch_n=n/2,ch_m=m/2;
		...
		for(int i=0;i<ch_n;i++) {
			for(int j=0;j<ch_m;j++) {
				new_arr[i][j]=arr[i][j+ch_m];
				new_arr[i+ch_n][j]=arr[i][j];
			}
			for(int j=ch_m;j<m;j++) {
				new_arr[i][j]=arr[i+ch_n][j];
				new_arr[i+ch_n][j]=arr[i+ch_n][j-ch_m];
			}
		}
		return new_arr;
	}
